...
Scientist.prototype.addDiscovery = function( discovery ) {
  ...
  if (this.discoveries.length >= 3){
    var lastDiscovery = this.discoveries.pop();
    
    return "I discovered " + this.discoveries.join(", ") + ", and " + lastDiscovery + ".";
...
Meal.prototype.containsJunkFood = function( ) {
  ...
  var canHasJunkFood = false;
  var myFoods = this.foods;

  junkFood.map( function (curr, index, array){
    if (myFoods.indexOf( curr ) !== -1){
      canHasJunkFood = true; 
    } 
  });

  return canHasJunkFood;
};
...
var notWarmOrColdAnimal = new Animal("Snuffleupagus").isWarmBlooded();
 ...
 *
 */
var openAClosedBox = new Box(george, false).openBox();
var closeAnOpenBox = new Box(new Animal('tiger'), true).openBox();
 ...
 *
 */
var farTooTallAStory = new House( 6 ).isATallStory( 2 );
...
var cookieMonsterBlocked = new Cookie("gingerbread").swipedByCookieMonster("Tuesday");
